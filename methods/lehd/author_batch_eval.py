#!/usr/bin/env python3
"""Official-style batched LEHD quality evaluation; never a BS1 timing result."""
...
from methods.lehd.config import (FORMAL_GPU, FORMAL_PROTOCOLS, UPSTREAM_COMMIT,
...
_AUTHOR_BATCH_REBOUND_FIELDS = {
    ...
    "batch_protocol_origin", "batch_override_reason", "rng_resume_semantics",
...
def _verify_runtime_algorithm_config(runtime_protocol: dict, author_protocol: dict) -> None:
    ...
    if expected != {key: author_protocol[key] for key in expected}:
        raise RuntimeError("LEHD author-batch protocol diverges from frozen BS1 algorithm config")


def batch_slices(count: int, batch_size: int) -> list[tuple[int, int]]:
    """Return ordered, non-empty original-instance batch ranges."""
...
def _comparison(left: float, right: float) -> dict:
    ...
    return {
        "left": left, "right": right, "abs_diff": abs(left - right),
...
def _cuda_device(torch, value):
    ...
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("LEHD author-batch evaluation requires CUDA")
...
def prepare_batch(problem: str, tasks: list, problem_size: int, *, device, torch):
    """Stack a real [B,...] ML4CO batch and return one official injection closure."""
    if not tasks:
        raise ValueError("author-batch preparation requires at least one task")
    if problem == "tsp":
        from methods.lehd.tsp.adapter import adapt_task, inject_official_data
        ...
        adaptations = [item[3] for item in adapted]
        return {
            "source_points": source_points, "adaptations": adaptations,
            "inject": lambda env: inject_official_data(
                env, model_points, references, device=device, torch=torch),
        }
    from methods.lehd.cvrp.adapter import adapt_task, inject_official_data
    ...
    if not np.array_equal(capacities, np.full_like(capacities, capacities[0])):
        raise ValueError(
            "LEHD CVRP author batch requires identical true capacities; official "
            "environment indexes raw_data_capacity[0]")
    return {
        "depots": [item[0] for item in adapted],
        "points": [item[1] for item in adapted],
        "raw_demands": [item[2] for item in adapted],
        "adaptations": [item[7] for item in adapted],
        "inject": lambda env: inject_official_data(
            env,
            np.stack([item[4] for item in adapted], axis=0),
...
def validate_batch(problem: str, tasks: list, prepared: dict, solutions: np.ndarray,
    ...
    """Decode and independently validate every output in a real official batch."""
    if len(tasks) != len(solutions) or len(tasks) != len(official_objectives):
        raise RuntimeError("LEHD batch output does not match the injected task count")
    if problem == "tsp":
    ...
    for local_index, (task, solution, official) in enumerate(
        ...
        if problem == "tsp":
            ...
            checked = independent_validate(
                ...
                prepared["raw_demands"][local_index],
                float(prepared["adaptations"][local_index]["capacity"]),
                decoded["canonical_solution"],
                capacity_tolerance=1e-5 * float(prepared["adaptations"][local_index]["capacity"]))
        if not checked["feasible"]:
            raise RuntimeError(f"LEHD independent validation failed at {dataset_offset + local_index}")
        ...
        official_check = _comparison(float(official), independent)
        kit_check = _comparison(independent, kit_objective)
        if not kit_feasible or not official_check["pass"] or not kit_check["pass"]:
            raise RuntimeError(f"LEHD objective/Kit validation failed at {dataset_offset + local_index}")
        reference = float(task.evaluate(task.ref_sol))
        if not np.isfinite(reference) or reference <= 0:
            raise ValueError(f"invalid LEHD reference at {dataset_offset + local_index}")
        records.append({
            "schema": "lehd-author-batch-record.v1", "method": "LEHD",
...
def main(argv=None):
    ...
    if dataset_sha != args.expected_dataset_sha256 or checkpoint_sha != args.expected_checkpoint_sha256:
        raise ValueError("LEHD author-batch asset SHA256 mismatch")
    project, upstream = git_provenance(ROOT), git_provenance(upstream_path)
    if project["dirty"]:
        raise ValueError("LEHD author-batch evaluation requires a clean project checkout")
    ...
    if len(tasks) != protocol["expected_dataset_count"]:
        raise ValueError("LEHD dataset count differs from frozen author-batch protocol")
    device = _cuda_device(torch, args.device)
    output_dir.mkdir(parents=True)
    metadata = {
        "schema": "lehd-author-batch-eval.v1", "state": "RUNNING",
    ...
    tester, runtime_protocol = build_tester(
        problem=args.problem, problem_size=args.problem_size, protocol_label=args.protocol,
    ...
    if len(records) != len(tasks):
        raise RuntimeError("LEHD author-batch evaluation did not validate every dataset instance")
    ...
    summary = {
        "schema": "lehd-author-batch-eval.v1", "status": "KIT_VALIDATED",
        "artifact_class": "baseline_result_reproduction", "method": "LEHD",
        "problem": protocol["problem"], "problem_size": protocol["actual_problem_size"],
        "protocol": protocol["protocol_label"], "RRC_budget": protocol["RRC_budget"],
