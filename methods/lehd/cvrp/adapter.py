"""Exact raw-unit ML4CO-to-LEHD CVRP boundary and route decoding."""
...
def _fields(task, *, problem_size: int):
    ...
    if (not np.array_equal(demands, np.rint(demands))
            or not float(capacity).is_integer()):
        raise ValueError("LEHD requires integral raw demands and true capacity")
...
def inject_official_data(env, coordinates, raw_demands, capacities, references,
    ...
    if (raw_demands.shape != (batch, nodes) or capacities.shape != (batch,)
        ...
        raise ValueError("LEHD CVRP injected tensors have inconsistent batch/size")
...
def decode_official_solution(solution, *, problem_size: int) -> dict:
    ...
    if values.shape != (problem_size, 2) or values.dtype.kind not in "iu":
        raise ValueError("captured LEHD CVRP solution must be integer [N,2]")
    ...
    if not np.array_equal(np.sort(nodes), np.arange(1, problem_size + 1)):
        raise ValueError("captured LEHD CVRP node order is not a customer permutation")
    if not np.isin(flags, (0, 1)).all() or not np.any(flags == 1):
        raise ValueError("captured LEHD CVRP route-start flags are invalid")
