#!/usr/bin/env python3
"""Export selected official ML4CO CVRPTW50 tasks to a neutral NPZ."""
...
from methods.mvmoe.cvrptw.config import (CAPACITY, DATASET_COUNT, DATASET_SHA256,
...
def main():
    ...
    if actual_hash != DATASET_SHA256:
        raise ValueError(f"unexpected CVRPTW50 dataset SHA256: {actual_hash}")

    import ml4co_kit as kit
    wrapper = kit.CVRPTWWrapper()
    wrapper.from_pickle(args.dataset)
    if len(wrapper.task_list) != DATASET_COUNT:
        raise ValueError(f"expected {DATASET_COUNT} CVRPTW50 tasks")
    ...
    for task in tasks:
        ...
        if task.points.shape != (PROBLEM_SIZE, 2) or task.tw.shape != (PROBLEM_SIZE + 1, 2):
            raise ValueError("source task does not have exact CVRPTW50 point/TW shapes")
        if task.service.shape != (PROBLEM_SIZE + 1,):
            raise ValueError("source task does not have exact CVRPTW50 service shape")
        if float(task.capacity) != CAPACITY:
            raise ValueError("unexpected CVRPTW50 capacity")
        ...
        if not all(np.isfinite(np.asarray(array)).all() for array in arrays):
            raise ValueError("CVRPTW50 task contains nonfinite values")
