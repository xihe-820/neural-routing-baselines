#!/usr/bin/env python3
"""Secondary ML4CO-Kit validation of MVMoE CVRPTW50 result JSON."""
...
from methods.mvmoe.cvrptw.config import DATASET_SHA256, PROBLEM_SIZE
...
def main():
    ...
    if dataset_hash != DATASET_SHA256:
        raise ValueError("validation dataset is not the pinned CVRPTW50 dataset")
    import ml4co_kit as kit
    wrapper = kit.CVRPTWWrapper()
    wrapper.from_pickle(args.dataset)
    failures = []
    for row in payload["results"]:
        if (row["problem"] != "CVRPTW" or row["problem_size"] != PROBLEM_SIZE or
                row["dataset_sha256"] != dataset_hash):
            raise ValueError("result identity does not match CVRPTW50 validation dataset")
