"""Select and canonicalize the exact official reward candidate."""
...
def decode_selected_nodes(selected_nodes, *, problem_size):
    ...
    if (raw < 0).any() or (raw > problem_size).any():
        raise ValueError("selected node ID outside 0..problem_size")
    if raw[0] != 0:
        raise ValueError("official MVMoE rollout must start at depot 0")
    # CVRPEnv keeps depot unmasked for finished POMOs. Its natural final depot
    # and any synchronization padding therefore form one trailing zero run.
    end = len(raw)
    ...
    if end == 0:
        ...
        canonical.append(0)
    return canonical, {"raw_steps": int(len(raw)),
