#!/usr/bin/env python3
"""Small-sample BS1 SIL timing probe with full post-timing correctness gates."""
...
from methods.sil.author_batch_eval import (_cuda_device, _load_tasks, _require_directory,
                                           _require_file, prepare_batch, validate_batch)
from methods.sil.config import (AUTHOR_BATCH_REGISTRY, FORMAL_PROTOCOLS,
...
from methods.sil.paper_results import TIMING_SEMANTICS


def timing_count(budget: str, count: int | None) -> int:
    selected = TIMING_PROBE_COUNTS[budget] if count is None else int(count)
    if selected <= 0:
        raise ValueError("SIL timing probe count must be positive")
    return selected
...
def _source_files(problem: str) -> list[dict]:
    return source_provenance([
        ...
        ROOT / "methods/sil/author_batch_eval.py",
...
def main(argv=None):
    ...
    count = timing_count(args.budget, args.count)
    if args.dump_config:
    ...
    if output.exists():
        raise ValueError("SIL timing probe output already exists")
    ...
    if dataset_sha != args.expected_dataset_sha256 or checkpoint_sha != args.expected_checkpoint_sha256:
        raise ValueError("SIL timing probe asset SHA256 mismatch")
    project, upstream = git_provenance(ROOT), git_provenance(upstream_path)
    if project["dirty"]:
        raise ValueError("SIL timing probe requires a clean project checkout")
    ...
    if len(tasks) != expected_count or count > len(tasks):
        raise ValueError("SIL timing probe dataset/count differs from frozen protocol")
    ...
    if runtime_protocol != protocol:
        raise RuntimeError("SIL timing runtime config differs from frozen protocol")
    ...
    summary = {
        "schema": "sil-bs1-timing-probe.v1", "status": "KIT_VALIDATED",
        "artifact_class": "baseline_bs1_timing_probe", "method": "SIL",
        "problem": protocol["problem"], "problem_size": protocol["actual_problem_size"],
        "protocol": args.budget, "budget": protocol["budget"], "count": count,
