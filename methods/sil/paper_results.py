"""Atomic SIL BS1 artifacts, exact resume identity, RNG state, and aggregation."""
...
SCHEMA = "sil-paper-production.v1"
TIMING_SEMANTICS = (
...
def initialize(output_dir: Path, identity: dict, *, resume: bool):
    ...
    if existed and resume and not metadata_path.is_file():
        raise ValueError("resume refused: existing SIL output lacks metadata.json")
    ...
    if metadata_path.exists():
        ...
        if (metadata.get("schema") != SCHEMA or metadata.get("identity") != identity
            ...
            raise ValueError("resume refused: SIL run identity differs")
        ...
        if len(records):
            if metadata.get("validated_records_sha256") != sha256_file(
                ...
                raise ValueError("resume refused: SIL validated records hash mismatch")
            if metadata.get("batch_timings_sha256") != sha256_file(
                    output_dir / "batch_timings.jsonl"):
                raise ValueError("resume refused: SIL timing hash mismatch")
        if [row["dataset_instance_index"] for row in records] != identity["indices"][:len(records)]:
            raise ValueError("resume refused: validated records are not an ordered prefix")
        if [row["dataset_instance_index"] for row in timings] != [
                row["dataset_instance_index"] for row in records]:
            raise ValueError("resume refused: timing and validated record indices differ")
        if len(records):
            checkpoint_path = output_dir / "checkpoint_state.json"
            if metadata.get("checkpoint_state_sha256") != sha256_file(checkpoint_path):
                raise ValueError("resume refused: SIL RNG checkpoint hash mismatch")
...
def append(output_dir: Path, metadata: dict, records: list, timings: list,
    ...
    if record["dataset_instance_index"] != expected_index:
        raise ValueError("SIL append is not the next expected dataset index")
    ...
    metadata["validated_records_sha256"] = sha256_file(output_dir / "validated_records.jsonl")
...
def finalize(output_dir: Path, metadata: dict, records: list, timings: list):
    ...
    if [row["dataset_instance_index"] for row in records] != identity["indices"]:
        raise ValueError("cannot finalize incomplete SIL run")
    if len(timings) != len(records):
        raise ValueError("cannot finalize SIL run with missing timings")
    for row in records:
        if (row.get("evidence_status") != "KIT_VALIDATED"
            ...
            raise ValueError("SIL record failed validation gate")
    objectives = np.asarray([_finite(r["independent_objective"], "objective") for r in records])
    refs = np.asarray([_finite(r["reference_objective"], "reference") for r in records])
    gaps = np.asarray([_finite(r["gap_percent"], "gap") for r in records])
    ...
    summary = {
        ...
        "identity": identity, "records_sha256": sha256_file(output_dir / "validated_records.jsonl"),
