...
from methods.sil.config import (AUTHOR_BATCH_REGISTRY, CHECKPOINTS, FORMAL_GPU,
                                FORMAL_PROTOCOLS, FORMAL_SIZES, SIZE_REGISTRY,
                                TIMING_PROBE_COUNTS, WARMUP_POLICY,
                                effective_repair_max, resolve_author_batch_config,
                                resolve_config,
                                validate_checkpoint_path, validate_dataset_path)
from methods.sil.author_batch_eval import (batch_slices as author_batch_slices,
...
from methods.sil.timing_probe import timing_count
...
class SILAuthorBatchProtocolTests(unittest.TestCase):
    class _ArrayTorch:
        float32 = np.float32
        long = np.int64
    ...
    def _tsp_task(offset=0.0):
        ...
        task = Task()
        task.points, task.ref_sol = points, np.asarray([0, 1, 2], dtype=np.int64)
    ...
    def _cvrp_task(offset=0.0, capacity=5):
        ...
        task = Task()
        task.depots, task.points, task.demands, task.capacity = (
            np.asarray([[0.0, 0.0]]), points, demands, capacity)
        task.ref_sol = np.asarray([0, 1, 2, 0, 3, 4, 0], dtype=np.int64)
    ...
    def test_author_batch_registry_exact_mappings_and_2k_labels(self):
        expected = {
            ("tsp", 1000): (128, 128), ("tsp", 2000): (64, 64),
            ("tsp", 5000): (32, 16), ("tsp", 10000): (16, 16),
            ("cvrp", 1000): (100, 100), ("cvrp", 2000): (100, 100),
        }
        self.assertEqual({key: (value["dataset_count"], value["batch_size"])
                          for key, value in AUTHOR_BATCH_REGISTRY.items()}, expected)
        for problem, size in (("tsp", 2000), ("cvrp", 2000)):
            protocol = resolve_author_batch_config(problem, size, "fewer")
            self.assertEqual(protocol["config_origin"], "senior_approved_adaptation")
            self.assertEqual(protocol["batch_protocol_origin"],
                             "adapted_from_author_1k_batch_cap")
        protocol = resolve_author_batch_config("tsp", 1000, "greedy")
        self.assertEqual(protocol["artifact_class"], "baseline_result_reproduction")
        self.assertEqual(protocol["original_instance_batch_size"], 128)
        self.assertEqual(protocol["batch_size_requested"], 128)
        with self.assertRaises(ValueError):
            resolve_author_batch_config("tsp", 1000, "greedy", batch_size=64)
        overridden = resolve_author_batch_config(
            "tsp", 1000, "greedy", batch_size=64, batch_override_reason="documented OOM")
        self.assertEqual(overridden["batch_size_requested"], 64)
        _verify_runtime_algorithm_config(resolve_config("tsp", 1000, "greedy"), protocol)
        bad_algorithm = dict(protocol)
        bad_algorithm["budget"] = 50
        with self.assertRaises(RuntimeError):
            _verify_runtime_algorithm_config(resolve_config("tsp", 1000, "greedy"), bad_algorithm)
    ...
    def test_batch_capture_shapes_objective_vector_and_timing_separation(self):
        ...
        timing_source = Path("methods/sil/timing_probe.py").read_text()
    ...
    def test_solve_batch_returns_one_solution_and_objective_per_original_instance(self):
        ...
        class Env:
            def _get_travel_distance_2(self, problems, candidate, **kwargs):
                self.seen = candidate
                return objective
        ...
        class Tester:
            def __init__(self):
                self.env = Env()
                self.env_params = {"k_nearest": 1, "decode_method": "greedy"}
                self.time_estimator_2 = types.SimpleNamespace(reset=lambda: None)

            def _test_one_batch(self, episode, batch_size, *args, **kwargs):
